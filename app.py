...
SAVED_IMPORT_DIR = DATA_DIR / "saved_imports"
APP_VERSION = "v15 CLEAN - cross-farm compare, fixed metrics, no old controls"
...
def save_uploaded_data_workbook(file_bytes: bytes) -> Path:
    """Save uploaded DATA workbook as the current backend and archive a dated copy."""
    DATA_DIR.mkdir(parents=True, exist_ok=True)
    BACKUP_DIR.mkdir(parents=True, exist_ok=True)
    SAVED_IMPORT_DIR.mkdir(parents=True, exist_ok=True)
    ...
    saved_path = SAVED_IMPORT_DIR / f"Amino_Eggsactly_Data_V1_import_{stamp}.xlsx"
    saved_path.write_bytes(file_bytes)

    return DATA_FILE


def latest_saved_import() -> Optional[Path]:
    if not SAVED_IMPORT_DIR.exists():
        return None
    files = sorted(SAVED_IMPORT_DIR.glob("Amino_Eggsactly_Data_V1_import_*.xlsx"), key=lambda x: x.stat().st_mtime, reverse=True)
...
def get_age_col(df: pd.DataFrame) -> str:
    return first_existing(list(df.columns), ["RoundDownAgeCalc", "Age in__weeks", "Flock_Age", "Age", "WOL"]) or "Age in__weeks"
...
def make_compare_graph(comparison_entries: List[Dict], standards: pd.DataFrame, selected_metrics: List[str], show_standards: bool, age_min: float, age_max: float) -> go.Figure:
    """Overlay multiple selected flocks on the same graph.

    Each entry should contain:
        {"label": str, "combined": DataFrame}
    Standards are plotted once. Actual traces are plotted once per flock/metric.
    Users can click items in the Plotly legend to hide/show whichever flock or
    metric they want without needing a separate metric selector.
    """
    std = standards[(standards["Graph_Age"] >= age_min) & (standards["Graph_Age"] <= age_max)].copy()
    ...
    metric_config = {
        "Production %": {"actual": "Production % Actual", "standard": "Production % Standard", "axis": "y"},
        "Mortality % cum": {"actual": "Mortality % cum Actual", "standard": "Mortality % cum Standard", "axis": "y2"},
        "Feed intake/day (g)": {"actual": "Feed intake/day (g) Actual", "standard": "Feed intake/day (g) Standard", "axis": "y3"},
        "Eggs/bird cum.": {"actual": "Eggs/bird cum. Actual", "standard": "Eggs/bird cum. Standard", "axis": "y4"},
        "Egg weight (g)": {"actual": "Egg weight (g) Actual", "standard": "Egg weight (g) Standard", "axis": "y5"},
        "Body weight (g)": {"actual": "Body weight (g) Actual", "standard": "Body weight (g) Standard", "axis": "y6"},
    }

    fig = go.Figure()
    max_actual_age = age_max
    for entry in comparison_entries:
        actual = entry.get("combined", pd.DataFrame())
        if actual is not None and not actual.empty and "Graph_Age" in actual.columns and actual["Graph_Age"].notna().any():
            max_actual_age = max(max_actual_age, float(actual["Graph_Age"].max()))
    add_period_shapes(fig, float(max(age_max, max_actual_age)))

    # One standards curve only, because the workbook has one default standard.
    for metric in selected_metrics:
        cfg = metric_config[metric]
        s_col = cfg["standard"]
        axis = cfg["axis"]
        if show_standards and s_col in std.columns and std[s_col].notna().any():
            fig.add_trace(
                go.Scatter(
                    ...
                    y=plot_series_without_zeros(std, s_col),
                    mode="lines",
                    name=f"Standard - {metric}",
                    line=dict(width=2, dash="dash"),
                    hovertemplate=f"Standard - {metric}: %{{y:.2f}}<extra></extra>",
                    connectgaps=True,
                    yaxis=axis,
                )
            )
                ...
                if actual_y.notna().any():
                    fig.add_trace(
                        go.Scatter(
                            ...
                            line=dict(width=2),
                            marker=dict(size=4),
                            hovertemplate=f"{label}<br>{metric}: %{{y:.2f}}<extra></extra>",
                            connectgaps=True,
                            yaxis=axis,
                        )
                    )
    ...
    fig.update_layout(
        height=760,
        margin=dict(l=70, r=340, t=55, b=120),
        xaxis=dict(title="Age in weeks", range=[age_min, age_max], dtick=4, domain=[0.0, plot_domain_right], hoverformat=".2f"),
        legend=dict(orientation="h", yanchor="bottom", y=-0.28, xanchor="center", x=0.5, itemclick="toggle", itemdoubleclick="toggleothers"),
        hovermode="x unified",
        hoverlabel=dict(align="left"),
        plot_bgcolor="white",
        paper_bgcolor="white",
...
st.markdown(
    ...
    <div class="hero">
        ...
        <div class="hero-subtitle">Review flock performance trends against standards and stitched rearing-to-layer history. v14 cross-farm compare mode loaded.</div>
    </div>
    """,
    unsafe_allow_html=True,
)
...
with st.sidebar:
    st.header("Backend")
    st.caption("v14 - clean compare controls loaded")
    ...
    if uploaded_data is not None:
        ...
        if is_valid:
            current_sig = file_signature(DATA_FILE)
            upload_sig = (uploaded_data.name, uploaded_data.size)
            saved_key = "last_saved_upload_sig"
            if st.session_state.get(saved_key) != upload_sig:
                save_uploaded_data_workbook(uploaded_bytes)
                st.session_state[saved_key] = upload_sig
                st.cache_data.clear()
                st.success("Workbook saved. Refreshing graphs...")
                st.rerun()
            else:
                st.success("Workbook already saved for this session.")
        else:
            st.error(message)
    ...
    latest_import = latest_saved_import()
    if latest_import is not None:
        st.write("**Last saved import**")
        st.caption(latest_import.name)
        st.caption(datetime.fromtimestamp(latest_import.stat().st_mtime).strftime("%d/%m/%Y %H:%M"))
    st.write("**Bridge file**")
    st.code(str(MATCH_FILE.relative_to(APP_DIR)))
    st.write("**Saved imports folder**")
    st.code(str(SAVED_IMPORT_DIR.relative_to(APP_DIR)))
...
# Build one clean selector table with one row per Farm/Flock combination. This
# supports cross-farm comparison, e.g. Farm 1 / Shed 3 versus Farm 2 / Shed 6.
selector_layers = (
    valid_layers[["Layer Farm", "Layer Flock"]]
    .dropna()
    .drop_duplicates()
    .sort_values(["Layer Farm", "Layer Flock"])
    .copy()
)
selector_layers["Selector Label"] = selector_layers["Layer Farm"].astype(str) + " | " + selector_layers["Layer Flock"].astype(str)
selector_label_to_pair = {
    row["Selector Label"]: (row["Layer Farm"], row["Layer Flock"])
    for _, row in selector_layers.iterrows()
}
all_selector_labels = selector_layers["Selector Label"].tolist()

c0, c1, c2, c3 = st.columns([0.75, 1.1, 1.1, 1.35])
with c0:
    compare_mode = st.checkbox(
        "Compare multiple flocks",
        value=False,
        help="Turn this on to overlay flocks from any farm on the same graph. Use the Plotly legend to hide/show individual lines.",
    )

# Latest partial week is excluded by default. No user control shown.
include_latest_partial_week = False

if compare_mode:
    default_compare = all_selector_labels[: min(3, len(all_selector_labels))]
    selected_compare_labels = st.multiselect(
        "Farm / flock combinations to compare",
        all_selector_labels,
        default=default_compare,
        help="Select any mix of farms/flocks. Example: Farm 1 | Shed 3 and Farm 2 | Shed 6.",
    )
    if not selected_compare_labels:
        st.info("Select at least one farm/flock combination to compare.")
        st.stop()

    selected_flock_pairs = [selector_label_to_pair[label] for label in selected_compare_labels]
    layer_farm, layer_flock = selected_flock_pairs[0]
    selected_flocks = [flock for _, flock in selected_flock_pairs]

    st.caption("Tip: single-click legend items to hide/show one line. Double-click a legend item to isolate it.")
else:
    with c1:
    ...
    if not flock_options:
        st.warning("No flocks found for this farm.")
        st.stop()

    with c2:
        layer_flock = st.selectbox("Flock", flock_options, index=0)

    selected_flock_pairs = [(layer_farm, layer_flock)]
    selected_flocks = [layer_flock]


# Fixed graph metrics — always shown by default.
# Use the Plotly legend to hide/show whatever lines are needed.
selected_metrics = [
    "Production %",
    "Mortality % cum",
    "Feed intake/day (g)",
...
comparison_entries = []
for farm, flock in selected_flock_pairs:
    c_combined, c_rear_raw, c_layer_raw, c_bridge_rows, c_removed = build_combined_life(
        data,
        bridge,
        layer_summary,
        rearing_summary,
        ...
        comparison_entries.append(
            {
                ...
                "combined": c_combined,
                "rear_raw": c_rear_raw,
                "layer_raw": c_layer_raw,
                "bridge_rows": c_bridge_rows,
                "partial_removed": c_removed,
            }
        )

if not comparison_entries:
    st.warning("No graph data found for the selected flock(s).")
    st.stop()

# Keep the original variable names populated from the first selected flock so the
# existing detail sections and KPIs still work in single-flock mode.
primary_entry = comparison_entries[0]
combined = primary_entry["combined"]
rear_raw = primary_entry["rear_raw"]
layer_raw = primary_entry["layer_raw"]
bridge_rows = primary_entry["bridge_rows"]
layer_partial_rows_removed = primary_entry["partial_removed"]

all_combined_for_age = pd.concat([e["combined"].assign(Compare_Farm=e["farm"], Compare_Flock=e["flock"], Compare_Label=e["label"]) for e in comparison_entries], ignore_index=True, sort=False)
max_actual_age = float(np.nanmax(all_combined_for_age["Graph_Age"])) if all_combined_for_age["Graph_Age"].notna().any() else 84.0

removed_total = sum(int(e.get("partial_removed", 0)) for e in comparison_entries)
if removed_total:
    st.info(f"Latest incomplete weekly layer row removed from the graph: {removed_total} source row(s).")
...
if compare_mode:
    ...
    for e in comparison_entries:
        c = e["combined"]
        latest_prod = c["Production % Actual"].dropna().iloc[-1] if "Production % Actual" in c and c["Production % Actual"].notna().any() else np.nan
        latest_birds = c["Birds"].dropna().iloc[-1] if "Birds" in c and c["Birds"].notna().any() else np.nan
        comparison_summary_rows.append(
            {
                "Farm": e["farm"],
                "Flock": e["flock"],
                "Max age": float(c["Graph_Age"].max()) if c["Graph_Age"].notna().any() else np.nan,
                "Latest production": latest_prod,
                "Latest birds": latest_birds,
                "Layer rows used": len(e["layer_raw"]),
                "Partial rows removed": e["partial_removed"],
            }
        )
    ...
    with k1:
        st.metric("Selected flocks", len(comparison_entries))
    with k2:
        st.metric("Max age", f"{max_actual_age:.2f} wks")
    with k3:
        st.metric("Latest production range", "-" if comparison_summary["Latest production"].dropna().empty else f"{comparison_summary['Latest production'].min():.2f}% - {comparison_summary['Latest production'].max():.2f}%")
    with k4:
        st.metric("Total layer rows", f"{sum(len(e['layer_raw']) for e in comparison_entries):,}")

    fig = make_compare_graph(comparison_entries, standards_graph, selected_metrics, show_standards, age_min, age_max)
...
# -----------------------------------------------------------------------------
if compare_mode:
    with st.expander("Comparison flocks used for this graph", expanded=True):
        st.dataframe(comparison_summary, use_container_width=True, hide_index=True)

    with st.expander("Combined comparison graph source data"):
        compare_data = pd.concat(
            [e["combined"].assign(Compare_Farm=e["farm"], Compare_Flock=e["flock"]) for e in comparison_entries],
            ignore_index=True,
            sort=False,
        )
        st.dataframe(compare_data, use_container_width=True, hide_index=True)
        csv = compare_data.to_csv(index=False).encode("utf-8")
        st.download_button(
            "Download comparison graph data CSV",
            data=csv,
            file_name="comparison_graph_cross_farm.csv",
            mime="text/csv",
        )

    with st.expander("Rearing-to-layer bridges used for comparison"):
        all_bridge_rows = []
        for e in comparison_entries:
            br = e["bridge_rows"].copy()
            if not br.empty:
                br.insert(0, "Compared Farm", e["farm"]); br.insert(1, "Compared Flock", e["flock"])
                all_bridge_rows.append(br)
        if all_bridge_rows:
            bridge_compare = pd.concat(all_bridge_rows, ignore_index=True, sort=False)
            display_cols = [c for c in ["Compared Farm", "Compared Flock", "Match Group ID", "Layer Farm", "Layer Flock", "Layer Birds", "Rearing Farm", "Rearing Flock", "Rearing Birds", "Confidence", "Graph Mode", "Allocation Note"] if c in bridge_compare.columns]
            st.dataframe(bridge_compare[display_cols], use_container_width=True, hide_index=True)
        else:
            st.info("No bridge rows found for the selected comparison flocks.")
